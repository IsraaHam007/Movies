...
Dataset : imdb_top_1000.csv (999 films original IMDb)
Sortie  : data/v1/
"""
...
# Chaque sous-genre est mappé vers une catégorie principale
# ----------------------------------------------------------------------------
...
# Mapping inverse : sous-genre -> catégorie majeure (ex: 'Horror' -> 'Crime_Horror')
SUBGENRE_TO_MAJOR = {sg: m for m, subs in GENRE_GROUPS.items() for sg in subs}

# Mapping catégorie -> ID numérique (ex: 'Drama' -> 0)
...
# Encodage numérique des sous-genres
...
# Affichage de la distribution des 4 catégories
...
# Affichage de la correspondance sous-genres -> catégorie
...
# Création de nouvelles colonnes dérivées pour enrichir le dataset
# ----------------------------------------------------------------------------
...
# Scores intermédiaires normalisés (utilisés pour construire les features composites)
...
# Score de popularité combiné (votes + rating + recettes)
...
# Ramène toutes les features entre 0 et 1 pour l'entraînement du modèle
# ----------------------------------------------------------------------------
...
# Alias lisibles pour les étapes suivantes
...
# Conversion de la liste en liste Python standard pour la sérialisation JSON
...
# Sauvegarde du mapping des genres (sous-genres + catégories majeures)
genre_mapping = {
...
# Sauvegarde d'un échantillon de 100 films pour tests rapides
