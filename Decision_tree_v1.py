"""
ARBRE DE DECISION — V1
Dataset : 999 films — dataset original IMDb
Prerequis : Data_preprocessing_v1.py doit avoir ete lance avant.
...
#
# On teste 3 normalisations differentes pour trouver la meilleure :
#
# MinMax : chaque colonne est ramenee entre 0 et 1 (deja fait en preprocessing)
#          L'arbre utilise les valeurs absolues pour definir ses seuils de coupure
#          Ex : "si IMDB_Rating_MM > 0.7 alors Drama"
#
# L1     : divise chaque ligne par la somme de ses valeurs absolues
#          La somme de chaque ligne vaut exactement 1.0
#          Permet de comparer des films par leurs proportions relatives de features
#
# L2     : divise chaque ligne par sa norme euclidienne (racine des carres)
#          La norme de chaque ligne vaut exactement 1.0
#          Met l'accent sur les features dominantes d'un film
#
# Note : L1 et L2 sont appliquees sur les donnees deja normalisees MinMax
# ----------------------------------------------------------------------------
...
# X_mm : features telles que produites par le preprocessing (MinMax)
X_mm = df[BASE_FEATURES].values

# X_l1 : normalisation L1 par ligne (somme de chaque ligne = 1)
X_l1 = normalize(X_mm, norm='l1')

# X_l2 : normalisation L2 par ligne (norme de chaque ligne = 1)
...
# Verification : la somme L1 et la norme L2 doivent valoir 1.0 pour la premiere ligne
...
#
# L'arbre de decision possede plusieurs hyperparametres qui influencent
# sa complexite et ses performances :
#
# max_depth        : profondeur maximale de l'arbre
#                    None = pas de limite (risque de surapprentissage)
#                    3-5  = arbre simple et generalisable
#
# min_samples_split: nombre minimum de films pour diviser un noeud
#                    Plus eleve = arbre plus simple, moins de surapprentissage
#
# min_samples_leaf : nombre minimum de films dans une feuille finale
#                    Plus eleve = predictions plus stables
#
# criterion        : mesure de qualite des coupures
#                    gini    = indice de Gini (par defaut, rapide)
#                    entropy = entropie de Shannon (information gain)
#
# GridSearchCV teste toutes les combinaisons possibles et retourne la meilleure
# cv=StratifiedKFold(5) : evaluation en 5-fold stratifie pour chaque combinaison
# n_jobs=-1 : utilise tous les coeurs du processeur pour accelerer la recherche
# ----------------------------------------------------------------------------
...
# On evalue les 3 normalisations avec les meilleurs hyperparametres trouves
...
# ----------------------------------------------------------------------------
# ENTRAINEMENT FINAL
# On entraine le modele avec les meilleurs hyperparametres
# et la meilleure normalisation identifiee ci-dessus
# ----------------------------------------------------------------------------
...
# get_depth()    : profondeur reelle de l'arbre apres entrainement
# get_n_leaves() : nombre de feuilles (noeuds finaux sans enfants)
# Un arbre profond avec beaucoup de feuilles peut indiquer du surapprentissage
...
# Meme logique que Naive Bayes : TP, FP, FN, TN calcules pour chaque genre
# ----------------------------------------------------------------------------
...
# L'arbre de decision calcule automatiquement l'importance de chaque feature
# en mesurant combien elle reduit l'impurete (Gini ou Entropie) lors des coupures
# Une feature avec une importance elevee = le modele s'appuie beaucoup dessus
...
# On charge les metriques du Naive Bayes (si disponibles) pour comparer
# les deux modeles sur les memes donnees
# Prerequis : Naive_Bayes_v1.py doit avoir ete lance avant
# ----------------------------------------------------------------------------
...
# On sauvegarde aussi la comparaison des 3 normalisations pour analyse
# ----------------------------------------------------------------------------
...
dt_info = {
    ...
    # Comparaison des 3 normalisations sauvegardee pour analyse
